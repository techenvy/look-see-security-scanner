...
if(getenv("REQUEST_METHOD") === "POST")
{
	...
	if($_POST["action"] == 'looksee_scan_settings')
	{
		...
		else
		{
			...
			$looksee_settings['looksee_inside'] = array_key_exists('looksee_inside', $_POST) && intval($_POST['looksee_inside']) === 1 ? 1 : 0;
			...
			<div class="postbox">
				...
				<div class="inside">
					<ul>
						...
						<li>
							<label><input type="checkbox" name="looksee_inside" value="1" <?php echo (looksee_get_option('looksee_inside') === true ? 'checked' : ''); ?> /> Analyze Content</label>
							<a href="#" class="settings-help" title="Click for more information" data-help="Malware tends to draw from a predictable subset of PHP functions.  When enabled, the scan will look for these functions inside all non-core PHP scripts.  This option requires additional memory resources and time to execute, so it is recommended you leave it disabled if your server is on the slow side.">[?]</a>
						</li>
<?php
...
elseif($scan_report['ended'] > 0)
{
	...
	$dbResult = $wpdb->get_results("SELECT `file`, `md5_expected`, `md5_found`, `wp`, `suspect_code` FROM `{$wpdb->prefix}looksee_files` WHERE (`suspect_code` OR NOT(`md5_expected`=`md5_found` OR (LENGTH(`md5_saved`) AND `md5_saved`=`md5_found`))) AND `skipped`=0 ORDER BY `file` ASC", ARRAY_A);
	if($wpdb->num_rows)
	{
		foreach($dbResult AS $Row)
		{
			...
			elseif(!strlen($Row["md5_found"]))
				$missing[] = $Row['file'];
			elseif($Row['md5_found'] !== $Row['md5_expected'])
				$altered[] = $Row['file'];

			if(intval($Row['suspect_code']) === 1)
				$analyzed[] = $Row['file'];
	...
	$info = array(
		...
		'missing_core'=>'The following core WordPress file(s) are missing and should be replaced with freshly downloaded copies, otherwise your site might not work correctly. Please note as of WordPress 3.8, some installations might be missing optional RTL-specific stylesheets; unless RTL language support is needed, it is safe to ignore them.',
		...
		'analyzed'=>'The following PHP script(s) were analyzed for functions commonly used by malware. Don\'t worry.  These same functions have perfectly legitimate uses too, so most of the results listed here are probably innocent.'
	);
?>
			<h3 class="hndle">Scan Results: <?php echo date("M j Y", floor($scan_report['ended'])); ?></h3>
				<div class="inside">
					...
					<ul id="looksee-scan-results">
						<?php
						foreach(array('altered','extra','missing','suspicious','old','skipped','analyzed') AS $status)
						{
							//only show content analysis results if it is enabled
							if($status === 'analyzed' && !looksee_get_option('looksee_inside'))
								continue;
							...
							elseif($status === 'analyzed')
								echo 'Found ' . count(${$status}) . ' file' . (count(${$status}) === 1 ? '' : 's') . ' w/ suspicious content';
							else
